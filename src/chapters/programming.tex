...
Następnie, przy pomocy komendy \inlinecode{ops.analysis('Static')} tworzy się obiekt analityczny odpowiedzialny za przeprowadzenie
...
Aby uwzględnić losowość parametrów modelu, stosuje się komendę w wersji skróconej \inlinecode{randomVariable(tag, dist, '-mean', mean, '-stdv', stdv)}. Komenda ta tworzy zmienne losowe, które umożliwiają wprowadzenie niepewności w parametrach modelu
...
Aby zdefiniować parametry modelu, które można później modyfikować w trakcie analizy, stosuje się komendę \inlinecode{parameter(tag, <specific parameter args>)}.
...
\begin{itemize}
    ...
    \item \textbf{property} – kontrolowana właściwość elementu (tekst) – \inlinecode{'E'} albo \inlinecode{'Z'}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{direction} – kierunek przyłożenia siły – \inlinecode{'wx'} albo \inlinecode{'wy'}
\end{itemize}

Aby rozszerzyć zakres wcześniej stworzonego parametru na wiele elementów/węzłów używa się komendy
\inlinecode{addToParameter(tag, <specific parameter args>)}. Jest to przydatne, gdyż w analizowanych konstrukcjach
...
\end{itemize}
Ostatnim krokiem parametryzacji modelu jest ustawienie transformacji prawdopodobieństwa na metodę Natafa \inlinecode{probabilityTransformation('Nataf', '-print', 0)}.
...
Za przeprowadzenie wielokrotnych symulacji analizy konstrukcji z uwzględnieniem losowości parametrów odpowiada funkcja \inlinecode{calculate}. Funkcja ta przyjmuje cztery kluczowe parametry:
...
\begin{enumerate}
    \item Funkcja rozpoczyna od uzyskania liczby zmiennych losowych \inlinecode{nrv} zdefiniowanych w modelu przy użyciu funkcji \inlinecode{ops.getRVTags()}. Ta liczba określa, ile losowych zmiennych zostanie wygenerowanych i zaktualizowanych w każdej symulacji.
    \item Następnie rozpoczyna się pętla, która wykonuje \inlinecode{n_trials} symulacji. Każda iteracja pętli reprezentuje jedną symulację z nowymi, losowo wygenerowanymi wartościami zmiennych.
    \begin{enumerate} [left=-12pt]
        \item \textbf{Resetowanie stanu analizy:} \inlinecode{ops.reset()} resetuje stan analizy, zapewniając, że każda symulacja rozpoczyna się od tych samych warunków początkowych.
        \item \textbf{Generowanie zmiennych losowych:} za pomocą \inlinecode{u = norm.rvs(size=nrv)} generowana jest lista \inlinecode{u} zawierająca \inlinecode{nrv} niezależnych zmiennych losowych z rozkładu normalnego.
        \item \textbf{Transformacja zmiennych:} \inlinecode{x = ops.transformUtoX(*u)} przekształca wygenerowane zmienne losowe \inlinecode{u} z przestrzeni standaryzowanej do przestrzeni fizycznej \inlinecode{x} przy użyciu transformacji Natafa.
        \item \textbf{Aktualizacja parametrów modelu:} iteracja \inlinecode{for index, _ in enumerate(x)} przechodzi przez każdą zmienną \inlinecode{x} i aktualizuje odpowiedni parametr modelu za pomocą \inlinecode{ops.updateParameter(index + 1, x[index])}. Każda zmienna \inlinecode{x} odnosi się do konkretnego parametru modelu (np. moduł sprężystości, obciążenie).
        \item \textbf{Przeprowadzenie analizy:} \inlinecode{ops.analyze(1)} wykonuje statyczną analizę konstrukcji na podstawie zaktualizowanych parametrów.
        \item \textbf{Obliczenie przemieszczenia:} \inlinecode{u_sims[i] = ops.nodeDisp(node_tag, dof)} oblicza przemieszczenie w zadanym węźle dla określonego stopnia swobody i zapisuje wynik do listy \inlinecode{u_sims} na odpowiednim indeksie.
    \end{enumerate}
\end{enumerate}
