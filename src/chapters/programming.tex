...
\begin{enumerate}
    \item Funkcja rozpoczyna od uzyskania liczby zmiennych losowych \ilc{nrv} zdefiniowanych w modelu przy użyciu funkcji \ilc{ops.getRVTags()}. Ta liczba określa, ile losowych zmiennych zostanie wygenerowanych i zaktualizowanych w każdej symulacji.
    \item Następnie rozpoczyna się pętla, która wykonuje \ilc{n_trials} symulacji. Każda iteracja pętli reprezentuje jedną symulację z nowymi, losowo wygenerowanymi wartościami zmiennych.
    \begin{enumerate} [left=-12pt]
        \item \textbf{Resetowanie stanu analizy:} \ilc{ops.reset()} resetuje stan analizy, zapewniając, że każda symulacja rozpoczyna się od tych samych warunków początkowych.
        \item \textbf{Generowanie zmiennych losowych:} za pomocą \ilc{u = norm.rvs(size=nrv)} generowana jest lista \ilc{u} zawierająca \ilc{nrv} niezależnych zmiennych losowych z rozkładu normalnego.
        \item \textbf{Transformacja zmiennych:} \ilc{x = ops.transformUtoX(*u)} przekształca wygenerowane zmienne losowe \ilc{u} z przestrzeni standaryzowanej do przestrzeni fizycznej \ilc{x} przy użyciu transformacji Natafa.
        \item \textbf{Aktualizacja parametrów modelu:} iteracja \ilc{for index, _ in enumerate(x)} przechodzi przez każdą zmienną \ilc{x} i aktualizuje odpowiedni parametr modelu za pomocą \ilc{ops.updateParameter(index + 1, x[index])}. Każda zmienna \ilc{x} odnosi się do konkretnego parametru modelu (np. moduł sprężystości, obciążenie).
        \item \textbf{Przeprowadzenie analizy:} \ilc{ops.analyze(1)} wykonuje statyczną analizę konstrukcji na podstawie zaktualizowanych parametrów.
        \item \textbf{Obliczenie przemieszczenia:} \ilc{u_sims[i] = ops.nodeDisp(node_tag, dof)} oblicza przemieszczenie w zadanym węźle dla określonego stopnia swobody i zapisuje wynik do listy \ilc{u_sims} na odpowiednim indeksie.
    \end{enumerate}
\end{enumerate}
